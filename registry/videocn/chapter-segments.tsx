...
/**
 * Le découpage de la piste en segments de chapitres.
 *
 * Ce fichier ne dessine rien. Il reçoit les couches du scrubber en `children`
 * et les réplique dans chaque segment : le scrubber reste propriétaire de ce
 * qui est peint — l'aperçu du buffer, la partie jouée — et n'a jamais à savoir
 * où tombent les coupures.
 *
 * **Chaque segment est une fenêtre sur la piste entière.** Le segment clippe ;
 * le calque qu'il contient reconstitue la largeur totale et se recale dessous.
 * Les couches répliquées lisent alors `--player-slider-fraction` et
 * `--player-buffer-start/end` telles quelles, sans une seule opération de plus.
 * C'est ce qui permet au curseur de garder un écrivain unique pour sa fraction :
 * un découpage qui aurait exigé une fraction par segment aurait demandé de la
 * réécrire autant de fois, soixante fois par seconde.
 *
 * **L'écart est retranché de la largeur du segment, jamais posé en marge.** Le
...
/**
 * L'écart entre deux segments, et l'absence d'écart après le dernier.
 *
...
export function ChapterSegments({ children }: ChapterSegmentsProps) {
  ...
  return (
    <div data-slot="video-player-scrubber-chapters" ref={containerRef} className="absolute inset-0">
      {segments.map((segment) => (
        ...
        <div
          ...
          // Un plancher de deux pixels : un chapitre plus court que l'écart
          // donnerait une largeur négative, donc un segment absent. Mieux vaut
          // une marque trop large qu'un trou dans la barre.
          className="absolute inset-y-0 left-[calc(var(--chapter-start,0)*100%)] w-[max(calc(var(--chapter-span,1)*100%_-_var(--chapter-gap,0px)),2px)] overflow-hidden rounded-full bg-foreground/20"
        >
          {/*
            Le calque : la piste entière, reconstituée à l'intérieur du segment
            et remontée sous sa fenêtre. Il est clippé par l'`overflow-hidden`
            ci-dessus, ce qui laisse ses couches raisonner en fractions de la
            vidéo entière, comme si les chapitres n'existaient pas.
          */}
          <div
